...
### Justificativa pela escolha da SAGA Coreografada

Ao escolher entre implementar uma saga coreografada e uma orquestrada para gerenciar as filas, optamos pela abordagem coreografada devido às suas vantagens significativas em termos de autonomia e simplicidade, especialmente em ambientes complexos. A nossa decisão foi guiada pela descentralização da lógica de decisão, onde cada serviço gerencia sua própria parte do processo. Essa autonomia permite que cada componente funcione de forma independente, o que facilita a evolução e a manutenção sem a necessidade de coordenação central.

Um dos principais benefícios dessa descentralização é a redução na complexidade. Na saga coreografada, não existe um orquestrador central que controla o fluxo de operações entre os serviços. Isso elimina a necessidade de gerenciar um componente adicional cuja função é dirigir a execução de tarefas entre diferentes serviços. Em contraste, na saga orquestrada, embora o controle centralizado possa parecer um benefício, ele traz o risco de se tornar um ponto único de falha. Se o orquestrador falhar, todo o processo pode ser comprometido, ao contrário da saga coreografada, onde o impacto de uma falha geralmente se limita ao serviço específico e suas interações diretas.

Adicionalmente, a abordagem orquestrada pode se tornar exponencialmente mais complexa à medida que novas regras de negócio são integradas. Cada nova regra pode exigir alterações no orquestrador, tornando a administração e atualização do sistema mais trabalhosas e propensas a erros. Nossa escolha pela coreografia, portanto, não apenas simplifica a arquitetura geral mas também promove uma escalabilidade mais eficiente e uma manutenção menos custosa, características essenciais para sistemas que visam flexibilidade e resiliência operacional.
